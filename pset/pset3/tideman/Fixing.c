...
bool check_cycle(int index)
{
    //look at pair loser check for its winn
    for (int i = index - 1; i >= 0; i--)
    {
        int check;
        check =  pairs[index].loser;
        printf("%s --> %s Cycle?\n", candidates[cycle_winner], candidates[pairs[index].loser]);
        
        if (pairs[i].winner == check)
        {
            if (pairs[i].loser == cycle_winner)
            {
                return false;
                break;
            }
